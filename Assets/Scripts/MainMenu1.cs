...
{
    public bool loadScreen, playOptions, loadOptions, options, brightness, resolution, controls;//these are all of out toggles to control what screen is on and when
    ...
    void OnGUI() //this section allows for you to render your GUI elements in screen
    {
        ...
        if (!loadScreen)//if the load screen is no longer active
        {
            if (!(playOptions || options))//and playoptions or options arent active then show the basic main menu screen
            {
                ...
                if (GUI.Button(new Rect(4 * scrW, 5.5f * scrH, 8 * scrW, 1 * scrH), "Options"))//options button on the menu screen
                {
            ...
            if (playOptions && !loadOptions)//inside the play options screen
            {
                ...
                if (loadOptions)//if load options are active then show	
                {
                    ...
                    int saveAmount = 4;//using this value as  fake amount of save files as a cheat speedy way to call files...yes this way does have issues but its effective for the example
  ...
  */
                ...
                if (GUI.Toggle(new Rect(1.5f * scrW, 4.5f * scrH, 2f * scrW, 0.5f * scrH), res3, "1280 x 720"))//if this toggle is pressed
                {
                ...
                if (GUI.Toggle(new Rect(1.5f * scrW, 6f * scrH, 2f * scrW, 0.5f * scrH), res6, "1920 x 1080"))//if this toggle is pressed
                {
